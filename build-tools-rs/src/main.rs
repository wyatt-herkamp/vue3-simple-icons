...
pub enum Commands {
    ...
    Build {
        ...
        #[clap(long = "types-location", default_value = "src/types")]
        types_location: PathBuf,
    },
}
#[tokio::main]
...
pub fn build_components(
    ...
    let types_components_folder = types_location.join("components");
    if !components_folder.exists() {
        std::fs::create_dir_all(&components_folder)?;
    }
    if !types_components_folder.exists() {
        std::fs::create_dir_all(&types_components_folder)?;
    }
    for icon in icons {
        ...
        std::fs::write(
            types_components_folder.join(format!("{}.vue.d.ts", component_name)),
            component_builder::COMPONENT_TYPE_DECLARATION,
        )?;
    ...
    }
    {
        ...
        let mut types_index_file = File::create(types_location.join("index.d.ts"))?;
        for component in &components.components {
            ...
            writeln!(
                types_index_file,
                r#"import {component_name} from "./components/{component_name}.vue";"#,
                component_name = component.component_name,
            )?;
        }
        ...
        writeln!(types_index_file, "export {{{}}};", exports)?;
    }
